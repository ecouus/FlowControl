...
show_port_list() {
    ...
    # 更新表头，增加流量使用、限制状态及自动重置周期
    echo -e "${BLUE}端口\t限额(GB)\t开始日期\t用户名\t流量使用\t是否被阻断\t重置周期${PLAIN}"
    echo -e "${BLUE}----------------------------------------------------------------------------${PLAIN}"
    ...
    while IFS=: read -r port limit_gb start_date user_name auto_reset_days || [[ -n "$port" ]]; do
        ...
        # 自动重置周期显示：0或空视为未开启
        local reset_cycle_display="未开启"
        if [ -n "$auto_reset_days" ] && [ "$auto_reset_days" -gt 0 ]; then
            reset_cycle_display="${auto_reset_days}天"
        fi

        echo -e "${GREEN}$port\t$limit_gb\t\t$start_date\t$user_name\t$usage_display\t$block_status\t$reset_cycle_display${PLAIN}"
        ((count++))
    done < "$CONFIG_FILE"
...
add_port_monitor() {
    ...
    # 询问是否开启自动重置功能
    local auto_reset_days=0
    read -p "是否开启自动重置功能？(y/n): " auto_reset_choice
    if [[ $auto_reset_choice =~ ^[Yy]$ ]]; then
        while true; do
            read -p "请输入自动重置天数（正整数）: " auto_reset_days
            if [[ $auto_reset_days =~ ^[0-9]+$ ]] && [ $auto_reset_days -gt 0 ]; then
                break
            else
                echo -e "${RED}无效的天数，请输入正整数。${PLAIN}"
            fi
        done
    fi

    # 添加监控，传入 auto_reset_days 作为第五个参数
    echo
    echo -e "${YELLOW}正在添加端口 $port 的监控配置...${PLAIN}"
    traffic-monitor add $port $limit $(date +%Y-%m-%d) "$user_name" $auto_reset_days
...
uninstall_system() {
    ...
    # 清除流量阻断表 (traffic_blocker)
    if nft list table inet traffic_blocker &>/dev/null; then
        echo -e "${YELLOW}清除流量阻断nftables表...${PLAIN}"
        nft flush table inet traffic_blocker
        nft delete table inet traffic_blocker
    fi
    
    # 清除流量监控表 (traffic_monitor)
    if nft list table inet traffic_monitor &>/dev/null; then
        echo -e "${YELLOW}清除流量监控nftables表...${PLAIN}"
    ...
    if command -v iptables-save >/dev/null 2>&1; then
        echo -e "${YELLOW}清除iptables监控规则...${PLAIN}"
        if [ -f "$CONFIG_FILE" ]; then
            while IFS=: read -r port _ || [[ -n "$port" ]]; do
                # 跳过注释或空行
                [[ $port =~ ^#.*$ || -z $port ]] && continue
                chain_name="TRACK_PORT_$port"
                echo -e "${YELLOW}- 清除端口 $port 的监控规则...${PLAIN}"
    ...
    # 清除iptables阻断规则 (针对配置文件中的端口和block_status.txt中记录的端口)
    echo -e "${YELLOW}清除iptables阻断规则...${PLAIN}"
    
    # 首先从block_status.txt中读取阻断端口
    if [ -f "$SCRIPT_DIR/block_status.txt" ]; then
        echo -e "${YELLOW}- 从阻断状态文件中清除规则...${PLAIN}"
        while IFS=: read -r port _ || [[ -n "$port" ]]; do
            ...
            iptables -D OUTPUT -p tcp --sport $port -j REJECT 2>/dev/null
            iptables -D OUTPUT -p tcp --sport $port -j DROP 2>/dev/null
        done < "$SCRIPT_DIR/block_status.txt"
    fi
    
    # 然后从config.ini文件中获取所有配置的端口并清除它们的阻断规则
    if [ -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}- 从配置文件中清除可能的阻断规则...${PLAIN}"
        while IFS=: read -r port _ || [[ -n "$port" ]]; do
            # 跳过注释或空行
            [[ $port =~ ^#.*$ || -z $port ]] && continue
    ...
    echo -e "${GREEN}流量监控系统已成功卸载! 所有相关规则已清理干净。${PLAIN}"
